...
import { getDefaultObjectTypeConfigs, } from './object-type-registry';
/**
...
export class PhoenixLoader {
    ...
    buildEventData(eventData, graphicsLibrary, ui, infoLogger) {
        ...
        const eventNumber = String(eventData['event number'] ?? eventData['eventNumber'] ?? '000');
        const runNumber = String(eventData['run number'] ?? eventData['runNumber'] ?? '000');
    ...
    loadObjectTypes(eventData) {
        ...
        for (const config of configs) {
            ...
            let extendUI;
            if (config.extendUI) {
                extendUI = (typeFolder, typeFolderPM) => {
                ...
                if (sc.scaleMethod === 'scaleJets') {
                    scaleFn = (value) => this.graphicsLibrary.getSceneManager().scaleJets(value);
                }
                else {
                    scaleFn = (value) => this.graphicsLibrary
                        .getSceneManager()
                        .scaleChildObjects(config.typeName, value, sc.scaleAxis);
                }
                extendUI = this.addScaleOptions(sc.key, sc.label, scaleFn);
            }
            this.addObjectType(data, getObject, config.typeName, config.concatonateObjs ?? false, config.cuts, extendUI);
        }
    }
    /**
    ...
    addObjectType(object, getObject, typeName, concatonateObjs = false, cuts, extendEventDataTypeUI) {
        ...
        for (const collectionName of collectionsList) {
            ...
            if (objectCollection.length == 0) {
                console.log(`Skipping ${typeName} collection ${collectionName} since it is empty.`);
                continue;
            }
            ...
            const collectionColor = new Color(object[collectionName][0].color
                ? object[collectionName][0].color
                : 0xffffff);
            this.ui.addCollection(typeName, collectionName, collectionCuts, collectionColor);
        }
