import { Vector3, Object3D, Color, CatmullRomCurve3, TubeGeometry, MeshToonMaterial, Mesh, BufferGeometry, LineBasicMaterial, Line, Group, Quaternion, CylinderGeometry, BufferAttribute, PointsMaterial, Points, BoxGeometry, MeshPhongMaterial, SphereGeometry, LineSegments, LineDashedMaterial, CanvasTexture, ShaderMaterial, DoubleSide, InstancedMesh, Matrix4, } from 'three';
...
export class PhoenixObjects {
    ...
    static getTracks(tracks) {
        ...
        for (const track of tracks) {
            ...
            if (track.pos) {
                track.pos = track.pos.filter((p) => p.every((v) => isFinite(v)));
            }
    ...
    static getTrack(trackParams) {
        ...
        const positions = trackParams.pos?.filter((p) => p.every((v) => isFinite(v)));
    ...
    static getJet(jetParams) {
        ...
        //
        const translation = new Vector3(0.5 * length * cphi * stheta + origin_X, 0.5 * length * sphi * stheta + origin_Y, 0.5 * length * ctheta + origin_Z);
     ...
     */
    static getCaloCellsInstanced(cellsParams) {
        const defaultRadius = 1700;
        const defaultZ = 2000;
        const defaultSide = 30;
        const defaultLength = 30;
        ...
        for (let i = 0; i < count; i++) {
            ...
            const position = PhoenixObjects.getCaloPosition(cell, defaultRadius, defaultZ);
