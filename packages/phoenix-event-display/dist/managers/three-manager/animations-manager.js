...
export class AnimationsManager {
     ...
     */
    constructor(scene, activeCamera, rendererManager, tweenGroup) {
        this.scene = scene;
    ...
    getCameraTween(pos, duration = 1000, easing) {
        const tween = new Tween(this.activeCamera.position, this.tweenGroup).to({ x: pos[0], y: pos[1], z: pos[2] }, duration);
        if (easing) {
            tween.easing(easing);
        }
        return tween;
    ...
    animateEvent(tweenDuration, onEnd, onAnimationStart) {
                ...
                if (eventObject.name === 'Track' || eventObject.name === 'LineHit') {
                    ...
                    if (geometryPosCount) {
                        ...
                        if (eventObject.geometry instanceof TracksMesh) {
                            ...
                            const eventObjectTween = new Tween(eventObject.material, this.tweenGroup).to({ progress: 1 }, tweenDuration);
                            ...
                            const eventObjectTween = new Tween(eventObject.geometry.drawRange, this.tweenGroup).to({ count: geometryPosCount }, tweenDuration);
                            eventObjectTween.onComplete(() => {
        ...
        const animationSphereTween = new Tween(animationSphere, this.tweenGroup).to({ radius: 3000 }, tweenDuration);
        ...
        const animationSphereTweenClone = new Tween(animationSphere, this.tweenGroup).to({ radius: 10000 }, extraAnimationSphereDuration);
    ...
    animateEventWithClipping(tweenDuration, onEnd, onAnimationStart, clippingConstant = 11000) {
        ...
        for (const animationClipPlane of animationClipPlanes) {
            ...
            const tween = new Tween(animationClipPlane, this.tweenGroup).to({ constant: clippingConstant }, tweenDuration);
