...
import { PerspectiveCamera, OrthographicCamera, Vector3, Group, InstancedMesh, TubeGeometry, MathUtils, } from 'three';
...
export class ControlsManager {
     ...
     */
    constructor(rendererManager, defaultView = [0, 0, 200], tweenGroup = new TweenGroup()) {
        /** Whether the overlay controls are linked to the main controls. */
        this._isOverlayLinked = false;
        ...
        const overlay_rendererElement = rendererManager.getOverlayRenderer()?.domElement;
        ...
        if (defaultView.length >= 6) {
            this.mainControls.target = new Vector3(defaultView[3], defaultView[4], defaultView[5]);
        }
    ...
    initOverlayControls() {
        if (this.renderr.getOverlayRenderer()?.domElement) {
            const overlay_rendererElement = this.renderr.getOverlayRenderer()
                ?.domElement;
            this.setOverlayControls(this.swapCameraType(this.getMainControls(), overlay_rendererElement));
        }
    }
    /**
    ...
    setupResize(rendererElement) {
        ...
        this.resizeHandler = () => {
            ...
            if (mainCamera instanceof OrthographicCamera) {
                const ratio = rendererElement.offsetWidth / rendererElement.offsetHeight;
     ...
     */
    getOverlayCamera() {
        return this.secondaryControls?.object;
    }
    // ====================================
     ...
     */
    CameraFactory(cameraClass, domElement) {
        if (cameraClass === OrthographicCamera) {
            return new OrthographicCamera(domElement.offsetWidth / -2, domElement.offsetWidth / 2, domElement.offsetHeight / 2, domElement.offsetHeight / -2, 10, 100000);
        }
        else if (cameraClass === PerspectiveCamera) {
            return new PerspectiveCamera(75, domElement.offsetWidth / domElement.offsetHeight, 10, 100000);
        }
    }
    /**
     ...
     */
    swapCameraType(input, overridden = input instanceof OrbitControls
        ? input.domElement
        : input, cameraType = input instanceof OrbitControls
        ? this.returnReverseType(input.object)
        : PerspectiveCamera, Iaspect = overridden.width / overridden.height) {
        const oldCamera = input instanceof OrbitControls ? input.object : null;
        const rendererElement = overridden;
        const ortbitTarget = input instanceof OrbitControls ? input.target : new Vector3(0, 0, 0);
    ...
    revertCameraType(camera) {
        if (camera === this.getMainCamera())
            ...
            this.setOverlayControls(this.revertCamerabyControl(this.getOverlayControls()));
    }
    // ====================================
     ...
     */
    syncPersFromOrtho(source, target, orbitTarget) {
     ...
     */
    syncOrthoFromPers(source, target, orbitTarget, forcedAspectRation) {
     ...
     */
    syncCameras(source, target, orbitTarget = new Vector3(0, 0, 0), forcedAspectRation = undefined) {
        ...
        else if (source instanceof OrthographicCamera &&
        ...
        else if (source instanceof PerspectiveCamera &&
            target instanceof OrthographicCamera) {
     ...
     */
    syncOverlayFromMain() {
        const overlayControls = this.swapCameraType(this.getMainControls(), this.getOverlayControls().domElement, this.returnType(this.getOverlayCamera()));
     ...
     */
    syncOverlayViewPort(widthRatio, heightRatio, mitigateMotion = true) {
        this.syncViewPort(this.getOverlayControls(), widthRatio, heightRatio, (this.isOverlayLinked()
            ? this.renderr.getOverlayRenderer()
            : this.getOverlayControls()).domElement, mitigateMotion);
    }
    /**
     ...
     */
    syncViewPort(control, widthRatio, heightRatio, overridenCanva, mitigateMotion = true) {
    ...
    switchContexts() {
        if (this.getOverlayControls()) {
            ...
            const overlayCameraCanvas = this.getMainControls()
                .domElement;
            const overlayControls = this.swapCameraType(this.getMainControls(), this.getOverlayControls().domElement, this.returnType(this.getMainCamera()));
            const mainControls = this.swapCameraType(this.getOverlayControls(), this.renderr.getMainRenderer().domElement, this.returnType(this.getOverlayCamera()));
    ...
    hideTubeTracksOnZoom(scene, minRadius) {
        ...
        if (this.controlsChangeHandler) {
            this.mainControls.removeEventListener('change', this.controlsChangeHandler);
        }
        // Reset tracks hidden state
        this.tracksHidden = false;
        const origin = new Vector3();
        // Store and add new change listener
        this.controlsChangeHandler = (event) => {
