...
export class RKHelper {
     ...
     */
    static extrapolateFromLastPosition(track, radius) {
        if (!track?.dparams)
            return [];
        ...
        const q = Math.round(p * qop);
        if (!startDir)
            startDir = CoordinateHelper.sphericalToCartesian(p, theta, phi).normalize();
        const inbounds = (pos) => Math.sqrt(pos.x * pos.x + pos.y * pos.y) <= radius;
